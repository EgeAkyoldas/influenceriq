...
import { fetchProfileByUsername } from '../src/lib/services/instagram-client';
import { analyzeProfile, type EditorExample } from '../src/lib/services/ai-analyzer';

async function processLeads() {
  console.log('🔄 Fetching all pending benchmark leads...');
  const pendingLeads = await getAll<{ id: number; username: string }>(
    "SELECT id, username FROM leads WHERE fetch_status = 'pending'"
  );
  ...
  for (const lead of pendingLeads) {
    count++;
    console.log(`\n[${count}/${pendingLeads.length}] Fetching @${lead.username}...`);
    ...
    try {
      ...
      if (!result) {
        console.log(`❌ @${lead.username} not found or unfetchable.`);
        await execute(
      ...
      console.log(`🤖 Running AI logic for @${lead.username}...`);
      try {
        ...
        if (analysis) {
          console.log(`✅ AI assigned Tier: ${analysis.tier}`);
          await execute(
            `INSERT INTO analysis_results (profile_id, primary_cluster, secondary_cluster, relevance_score, authority_score, engagement_rate, monetization_signals, risk_flags, dynamic_tags, content_style, audience_alignment, tier, tier_reason, content_summary, is_approved, rejection_reason)
             ...
             ON CONFLICT(profile_id) DO UPDATE SET
               ...
               secondary_cluster = excluded.secondary_cluster, relevance_score = excluded.relevance_score,
               authority_score = excluded.authority_score, monetization_signals = excluded.monetization_signals,
               risk_flags = excluded.risk_flags, dynamic_tags = excluded.dynamic_tags,
               content_style = excluded.content_style, audience_alignment = excluded.audience_alignment,
               tier_reason = excluded.tier_reason, content_summary = excluded.content_summary,
               is_approved = excluded.is_approved, rejection_reason = excluded.rejection_reason,
               analyzed_at = datetime('now')`,
            [
              profileId, analysis.primary_cluster, analysis.secondary_cluster || null,
              analysis.relevance_score || 0, analysis.authority_score || 0, engRate,
              JSON.stringify(analysis.monetization_signals || []),
              JSON.stringify(analysis.risk_flags || []), JSON.stringify(analysis.dynamic_tags || []),
              analysis.content_style || 'Mixed',
              analysis.audience_alignment || 0, analysis.tier || 'D', analysis.tier_reason || '',
              analysis.content_summary || '', analysis.is_approved ? 1 : 0, analysis.rejection_reason || null
            ]
          );
        ...
        console.error(`❌ AI fail @${lead.username}:`, aiErr.message);
      }
      ...
      console.log(`✅ Lead finished: @${lead.username}`);
    } catch (error: unknown) {
      const err = error as Error;
      const msg = err.message;
      if (msg.includes('Rate Limit Exceeded') || msg.includes('429')) {
        console.error(`[Fetch] 🛑 Fatal Rate Limit encountered on @${lead.username}. Aborting fetch loop to protect API key.`);
        await execute("UPDATE leads SET fetch_status = 'pending' WHERE id = ?", [lead.id]);
        break;
      }

      await execute("UPDATE leads SET fetch_status = 'error', error_message = ?, updated_at = datetime('now') WHERE id = ?", [msg, lead.id]);
      console.error(`❌ Fetch error @${lead.username}:`, msg);
    }
  }
