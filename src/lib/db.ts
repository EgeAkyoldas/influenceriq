...
async function ensureSchema() {
  ...
  try {
    const stmts = [
      ...
      `CREATE TABLE IF NOT EXISTS analysis_snapshots (id INTEGER PRIMARY KEY AUTOINCREMENT, profile_id INTEGER NOT NULL, snapshot_reason TEXT DEFAULT 'reverify', primary_cluster TEXT, secondary_cluster TEXT, relevance_score REAL, authority_score REAL, engagement_rate REAL, monetization_signals TEXT DEFAULT '[]', audience_alignment REAL, tier TEXT, tier_reason TEXT, content_style TEXT, content_summary TEXT, is_approved INTEGER, rejection_reason TEXT, snapshotted_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (profile_id) REFERENCES profiles(id) ON DELETE CASCADE)`,
