...
async function ensureSchema() {
  ...
  try {
    const stmts = [
      ...
      `CREATE TABLE IF NOT EXISTS reverify_jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, total_profiles INTEGER DEFAULT 0, processed INTEGER DEFAULT 0, errors INTEGER DEFAULT 0, status TEXT DEFAULT 'pending' CHECK(status IN ('pending','running','complete','cancelled')), started_at TEXT, completed_at TEXT, created_at TEXT DEFAULT (datetime('now')))`,
