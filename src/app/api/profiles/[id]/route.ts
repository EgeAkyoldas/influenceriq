  ...
  try {
    ...
    const profile = await getOne(`
      SELECT p.*,
        ...
        (
          SELECT s.snapshotted_at FROM analysis_snapshots s
          WHERE s.profile_id = p.id
            AND s.snapshot_reason IN ('reverify', 'batch_reverify')
          ORDER BY s.snapshotted_at DESC LIMIT 1
        ) as last_reverified_at
      FROM profiles p
    ...
    const parsed = {
      ...
      last_reverified_at: (p.last_reverified_at as string) || null,
